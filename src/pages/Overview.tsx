...
export const Overview = () => {
  ...
  const { budgetAndBeneficiaries, programs, loading } = useSelector(
    (state: IPrograms) => state.program
  );
  useEffect(() => {
  ...
  const calculateSize = (programs: any) => {
    const sdgs = uniqBy(
      flatten(programs?.map(({ sdgs }: any) => sdgs)),
      "sdgId"
    );
    return sdgs || [];
  };
            ...
            <div className="row page-title-header">
              <div className="col-12">
                ...
                <div className="dashboard-card">
                  <OverviewStat
                    budgetAndBeneficiaries={budgetAndBeneficiaries}
                    programs={programs}
